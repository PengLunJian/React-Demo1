...
import { getChannelPurchase, clearChannelPurchase } from '../../../redux/ducks/channel/purchase';
import List from '../../../components-WX/channel/details/purchase/List';
...
class ChannelPurchase extends Component {
  ...
  loadData = (loadMore) => {
    ...
    if (!loadMore) {
      this.props.clearChannelPurchase();
    }
    return new Promise((resolve, reject) => {
      this.props.getChannelPurchase(this.params);
      resolve();
    });
  };
...
const mapStateToProps = (state) => {
  const { channel } = state;
  const { channelPurchase } = channel;
  const { data, totalCount, isLoading } = channelPurchase;
